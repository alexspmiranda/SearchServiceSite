...
<?php include_once("header.php"); ?>
    ...
    <article class="container-principal">
    	...
    	<div class="carregando">            
			...
			$resultado_respondidosAdmin = sds_selectRespondidosInbox($pag);
			   	
			$cont = 0;
			foreach($resultado_respondidosAdmin as $res_respondidosAdmin){
				$emailId        = $res_respondidosAdmin['contatoId'];
				$emailNome      = $res_respondidosAdmin['contatoNome'];
				$emailEmail     = $res_respondidosAdmin['contatoEmail'];
				$emailAssunto     = $res_respondidosAdmin['contatoAssunto'];
				$emailMensagem  = $res_respondidosAdmin['contatoMensagem'];
				$emailData      = $res_respondidosAdmin['dataMensagem'];
				$statusMensagem    = $res_respondidosAdmin['statusMensagem'];
				$emailResposta  = $res_respondidosAdmin['respostaMensagem'];
				$i++;
				if($i % 2 == 0){
            ...
            <?php
				...
				<a href=\"painel.php?exe=admin-inbox/respondidos&amp;pag=1\">Primeira Página</a>&nbsp;&nbsp;&nbsp;";
				
				for ($i = $pag-$links; $i <= $pag-1; $i++){
				if ($i <= 0){
				}else{
				echo"<a href=\"painel.php?exe=admin-inbox/respondidos&amp;pag=$i\">$i</a>&nbsp;&nbsp;&nbsp;";
				}
				}echo "$pag &nbsp;&nbsp;&nbsp;";
				...
				echo "<a href=\"painel.php?exe=admin-inbox/respondidos&amp;pag=$paginas\">Última página</a>&nbsp;&nbsp;&nbsp;
